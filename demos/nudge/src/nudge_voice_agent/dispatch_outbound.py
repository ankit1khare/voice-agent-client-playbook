...
def build_dispatch_request(
    ...
    *,
    ...
    return api.CreateAgentDispatchRequest(
        ...
        room=f"nudge-outbound-{call.request_id}",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preview or dispatch one authorized synthetic Nudge outbound call."
    )
    ...
    parser.add_argument(
        "--agent-name",
        default=os.getenv("NUDGE_OUTBOUND_AGENT_NAME", OUTBOUND_AGENT_NAME),
    )
    parser.add_argument(
        "--record-file",
        type=Path,
...
def main() -> None:
    ...
    if (
        args.execute
        and os.getenv("NUDGE_ENABLE_OUTBOUND_CALLS", "false").lower() != "true"
    ):
        raise SystemExit("set NUDGE_ENABLE_OUTBOUND_CALLS=true before using --execute")
