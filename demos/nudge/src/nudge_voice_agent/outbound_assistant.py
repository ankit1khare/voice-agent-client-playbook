"""Nudge's outbound demo prompt and deterministic messages."""

from nudge_voice_agent.call_control import (
...
def outbound_disclosure() -> str:
    """Return the fixed Nudge-approved opening for a human answer."""
    return (
        "Hi, this is Nudge, an AI assistant calling about a document request. "
...
class NudgeOutboundDocumentCollectionAssistant(WorkflowAgent):
    """Outbound assistant for a synthetic Nudge document reminder."""
...
def outbound_assistant_instructions(
    ...
    return f"""You are Nudge, an AI assistant in a synthetic outbound demonstration.
...
- If it is a good time, say you are calling for {record.business_name}, then ask
  ...
  Nudge document requests. Nudge has approved naming the business at this stage.
...
- If the listener is the wrong person, say you cannot discuss the request. Ask them
  to have {record.contact_name} call {NUDGE_SUPPORT_PHONE_SPOKEN}, then end politely.
...
- Give the real Nudge upload path from this synthetic record:
...
- If the listener is already working with a Nudge employee, ask for the employee's
  name and the latest status, then call record_existing_nudge_contact. Do not claim
  an existing ticket or case was updated.
- If the listener requests a person, call request_human_transfer. Live transfer is
  disabled for this demo. Offer {NUDGE_SUPPORT_PHONE_SPOKEN} or {NUDGE_SUPPORT_EMAIL}.
...
- For anything not covered here, direct the listener to {NUDGE_SUPPORT_EMAIL} or
  {NUDGE_SUPPORT_PHONE_SPOKEN}. General help is at {NUDGE_HELP_CENTER_URL}.
...
def voicemail_message(record: CentralizedCallRecord = DEMO_CALL_RECORD) -> str:
    ...
    return (
        f"Hi {record.contact_name.split()[0]}, this is Nudge, an AI assistant "
        ...
        f"upload the documents through the Nudge platform by going to "
        ...
        f"{NUDGE_SUPPORT_EMAIL} or call {NUDGE_SUPPORT_PHONE_SPOKEN}. "
        "Thank you for your attention, and have a wonderful day."
    )


def follow_up_tool_for(
    agent: NudgeOutboundDocumentCollectionAssistant,
) -> FollowUpPreviewTool:
