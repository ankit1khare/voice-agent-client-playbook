"""Nudge, the Nudge document collection demo assistant."""

from nudge_voice_agent.call_control import build_end_call_tool
from nudge_voice_agent.demo_context import (
    DEMO_CALL_RECORD,
    LARGE_FILE_HELP,
    NUDGE_HELP_CENTER_URL,
    NUDGE_SUPPORT_EMAIL,
    NUDGE_SUPPORT_PHONE_SPOKEN,
    CentralizedCallRecord,
    render_demo_record,
)
from nudge_voice_agent.workflow import (
    FollowUpPreviewTool,
    build_follow_up_preview_tool,
)
from nudge_voice_agent.workflow_agent import WorkflowAgent

INITIAL_DISCLOSURE = (
    "Hi, I'm Nudge, your AI paperwork assistant. This call is being recorded. "
    "I can help "
...
class NudgeDocumentCollectionAssistant(WorkflowAgent):
    """Inbound assistant for the synthetic Nudge document collection demo."""
...
def assistant_instructions(
    ...
    """Return Nudge's grounded conversation instructions."""
    return f"""You are Nudge, an AI paperwork assistant in an inbound demonstration.
The caller contacted Nudge about a required document. The application already spoke
...
- If verification fails, ask the caller to restate or spell the full business name.
  Call verify_business_name again with each restated or spelled name.
...
- Never claim you can see their screen, inspect their account, or confirm that Nudge
  received the upload.
...
- For an extension, use record_extension_request only after the caller supplies
  the requested submission date. If they ask for more time without a date, ask
  "What date would you like to request?" and wait for their answer. Do not call
  the tool yet. Never fill the date with unknown, TBD, or an invented date. If
  the tool rejects a missing date, ask for it and wait before retrying. Say
  Underwriting will review the request. Never promise approval.
- For a disputed deadline, ask which deadline the caller expected and call
  record_deadline_dispute. After it succeeds, say Nudge will need to check the
...
- If the caller is already working with a Nudge employee, ask for the employee's name
  and the latest status, then call record_existing_nudge_contact. Do not claim an
  existing case was updated.
- If the caller requests a person, call request_human_transfer. Live transfer is
  disabled in this demo. Offer {NUDGE_SUPPORT_PHONE_SPOKEN} or {NUDGE_SUPPORT_EMAIL}.
...
- Do not invent Nudge policies, account facts, navigation steps, or document status.
...
- For questions not answered here, say you do not have that information and direct
  the caller to {NUDGE_SUPPORT_EMAIL} or {NUDGE_SUPPORT_PHONE_SPOKEN}.
- The general support source is {NUDGE_HELP_CENTER_URL}.
...
def follow_up_tool_for(agent: NudgeDocumentCollectionAssistant) -> FollowUpPreviewTool:
