"""Tests for Nudge LiveKit session configuration and disclosure playback."""
...
from nudge_voice_agent.assistant import INITIAL_DISCLOSURE
...
from nudge_voice_agent.settings import Settings
...
def test_build_session_model_config_uses_coda_voice() -> None:
...
def test_build_turn_handling_options_tunes_phone_conversation() -> None:
...
def test_build_room_options_enables_audio_enhancement() -> None:
    options = build_room_options()

    assert isinstance(options, room_io.RoomOptions)
    assert isinstance(options.audio_input, room_io.AudioInputOptions)
    assert options.audio_input.noise_cancellation is not None
    assert options.audio_input.pre_connect_audio is False


def test_disclosure_uses_exact_non_interruptible_speech() -> None:
