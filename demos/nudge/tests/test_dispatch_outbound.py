...
from nudge_voice_agent import dispatch_outbound
from nudge_voice_agent.demo_context import DEMO_CALL_RECORD
from nudge_voice_agent.dispatch_outbound import (
    build_dispatch_request,
)
from nudge_voice_agent.outbound import OutboundCallRequest
...
def test_dispatch_routes_authorized_metadata_to_outbound_agent() -> None:
    ...
    assert request.agent_name == "nudge-demo"
    assert request.room == "nudge-outbound-demo-5678"
    assert metadata == {
        "authorized_test_call": True,
        "call_record": DEMO_CALL_RECORD.to_dict(),
        "demo_only": True,
        "mode": "nudge_outbound_test",
        "phone_number": "+14155550123",
        "request_id": "demo-5678",
    }
...
def test_cli_previews_masked_destination_without_dispatching(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["nudge-outbound-dispatch", "--phone-number", "+14155550123"],
    )
    ...
    assert '"call_record_id": "nudge-demo-sep10-001"' in output
...
def test_cli_execute_requires_environment_switch(
    ...
    monkeypatch.delenv("NUDGE_ENABLE_OUTBOUND_CALLS", raising=False)
    monkeypatch.setattr(
        sys,
        "argv",
        [
    ...
    with pytest.raises(SystemExit, match="NUDGE_ENABLE_OUTBOUND_CALLS=true"):
        dispatch_outbound.main()
...
def test_cli_never_dispatches_to_client_service(
    ...
    monkeypatch.setenv("NUDGE_ENABLE_OUTBOUND_CALLS", "true")
