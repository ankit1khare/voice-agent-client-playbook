...
def test_outbound_prompt_requires_authorization_before_document_details() -> None:
    ...
    assert "Nudge has approved naming the business at this stage" in instructions
...
def test_runtime_voicemail_matches_rendered_demo_asset_text() -> None:
    asset = (
        Path(__file__).parents[1] / "assets" / "nudge_voicemail_reminder.txt"
    ...
    assert "support@nudge.example" in asset
    assert "service@nudge.com" not in asset
    ...
    assert "Nudge Demo Portal" not in asset
