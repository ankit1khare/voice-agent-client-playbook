...
from nudge_voice_agent.demo_context import DEMO_CALL_RECORD
...
def test_extension_preview_has_required_fields_and_no_external_write() -> None:
    ...
    assert preview == {
        ...
        "request_id": "demo-extension",
        "call_record_id": "nudge-demo-sep10-001",
...
def test_human_transfer_preview_never_claims_transfer_performed() -> None:
    ...
    assert preview["human_transfer"] == {
        "requested": True,
        "performed": False,
        "destination": "Nudge Client Service",
    }
...
def test_preview_tool_logs_and_retains_each_payload(
    ...
    with caplog.at_level(logging.INFO, logger="nudge-workflow-preview"):
        preview = tool.record_preview(
            ConversationDisposition.EXISTING_NUDGE_CONTACT,
            nudge_contact_name="Lucas",
...
def test_document_request_details_are_spoken_as_one_grounded_response() -> None:
    ...
    assert ctx.session.messages == [
        "Nudge is still awaiting August 2026 bank statements and second quarter "
...
def test_outbound_document_reminder_ends_with_nudges_submission_question() -> None:
...
def test_interrupted_tool_result_requests_complete_recovery() -> None:
    ...
    assert "support@nudge.example" in pending
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "record_existing_nudge_contact",
            {
                "nudge_contact_name": "Lucas",
                "status_update": "the documents are being prepared",
            },
            "existing_nudge_contact",
            {
                "nudge_contact_name": "Lucas",
                "status_update": "the documents are being prepared",
            },
...
@pytest.mark.parametrize(
    "missing_date",
    ["unknown", " UNKNOWN. ", "TBD", "not provided", "N/A", "more time", "soon", "???"],
)
def test_missing_extension_date_leaves_no_request_and_can_recover(
    missing_date: str,
) -> None:
    tool = FollowUpPreviewTool(DEMO_CALL_RECORD, "demo-missing-extension-date")
    ctx = _FakeRunContext()
    asyncio.run(cast(Any, tool.verify_business_name)(None, "Northstar Labs"))
    action = cast(Any, tool.record_extension_request)

    with pytest.raises(ToolError, match="Ask the caller"):
        asyncio.run(action(ctx, missing_date))

    assert tool.previews == []
    assert ctx.session.messages == []
    assert tool.take_pending_spoken_result() is None

    asyncio.run(action(ctx, "September 25, 2026"))

    assert len(tool.previews) == 1
    assert (
        tool.previews[0]["details"]["requested_submission_date"] == "September 25, 2026"
    )
    assert len(ctx.session.messages) == 1
    assert "It is not approved yet" in ctx.session.messages[0]
